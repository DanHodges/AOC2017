const matchesNext = (item, index, array) =>
  item === array[(index + 1) % array.length];

const matchesHalfwayAround = (item, index, array) =>
  item === array[(index + array.length / 2) % array.length];

const sum = (a, b) => a + b;

const solution = filter => data =>
  Array.from(data)
